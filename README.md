...
The survey is generated from a json structure.
The json can be mocked 1) or retrieved from an remote endpoint 2).
The first use case is the default one. See the file mockQuestions.json in /src.
If the json is fetched from a remote endpoint, please do it in the
componentDidMount() method in the container QuestionList.

The Json should adhere to the following rules:

[{
 "text": String,
 "type": FormType
  "choices": [{"id": Id, value: "String"}]
}]
...
Please see the mockQuestions.json it has all use cases.

-The survey can be browsed using the native browser back / next button <br/>
-The progress is saved in localstorage so  When closing the browser window and reopening it, the progress with its
data is restored <br/>

**To improve:**

A11sys,Add tests,Keep working on the scss

**Here is the live version:**

https://mrfrej.github.io/react-survey-tool/
